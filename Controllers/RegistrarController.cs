...
public class RegistrarController : ControllerBase
{
    ...
    public RegistrarController(IRegistrarService registrarService, ITenantService tenantService, ILogger<TeachersController> logger)
    {
    ...
    public async Task<IActionResult> GetAll()
    {
        ...
        {   var requestTenantId = _tenantService.GetCurrentTenantId();
            ...
            var collection = await _registrarService.GetAllRegistrarsAsync();
    ...
    public async Task<IActionResult> GetById(string id)
    {
        try
        {
            var profile = await _registrarService.GetRegistrarByIdAsync(id);
            return Ok(profile);
        }
    ...
    [HttpPost]
    public async Task<IActionResult> Register([FromBody] RegistrarUpsertDto payload)
    {
        try
        {
            var result = await _registrarService.RegisterRegistrarAsync(payload);
    ...
    [HttpPut("{id}")]
    public async Task<IActionResult> Update(string id, [FromBody] RegistrarUpsertDto payload)
    {
        try
        {
            await _registrarService.UpdateRegistrarAsync(id, payload);
            return NoContent();
        }
        catch (KeyNotFoundException ex)
        {
    ...
    public async Task<IActionResult> Delete(string id)
    {
        try
        {
            await _registrarService.DeleteRegistrarAsync(id);
            return NoContent();
        }
        catch (KeyNotFoundException ex)
        {
